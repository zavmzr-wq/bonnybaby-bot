...
TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN", "8789819533:AAFgRVm5odAIdg6fxEZu1cRK2Glrp3Z73TE")
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "AQ.Ab8RN6KHHnj-ZEIo73N_VxG8hey0-3tUKHx-Ky24bfggSvJYwg")
...
Если вопрос не по теме — вежливо переведи разговор на услуги Bonnybaby."""


# ───────────────────────────────────────────────
# AI-ответ через Gemini (бесплатно)
# ───────────────────────────────────────────────
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text(
            "🛒 *Каталог товаров*\n\nВыберите товар для просмотра цен:",
            parse_mode="Markdown",
        ...
        await query.edit_message_text(
            TEXT_CONDITIONS, parse_mode="Markdown", reply_markup=back_keyboard()
        )
    elif data == "schedule":
        await query.edit_message_text(
            TEXT_SCHEDULE, parse_mode="Markdown", reply_markup=back_keyboard()
        )
    elif data == "contacts":
        await query.edit_message_text(
            TEXT_CONTACTS, parse_mode="Markdown", reply_markup=back_keyboard()
        )
    elif data == "delivery":
        await query.edit_message_text(
            TEXT_DELIVERY, parse_mode="Markdown", reply_markup=back_keyboard()
        )
    elif data == "ask":
        await query.edit_message_text(
